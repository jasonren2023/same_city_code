...
style='background-color: #FFF;border: 1px solid #DDD;color: inherit;float: left;line-height: 1.42857;margin: -1px;position: relative;text-decoration: none;height: 33px;width: 80px;outline: none;'></li>";
...
function tomedia($src, $local_path = false, $is_cahce = false) {
    ...
    //本公众号设置信息
    $wlsettings = pdo_get('wlmerchant_setting', array('key' => 'enclosure', 'uniacid' => $_W['uniacid']), array('value'));
    if (is_array($wlsettings)) {
        $_W['wlsetting']['enclosure'] = iunserializer($wlsettings['value']);
        if($_W['wlsetting']['enclosure']['service'] > 0){
