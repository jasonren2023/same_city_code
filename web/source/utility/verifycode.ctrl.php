...
if ('send_code' == $do) {
    $row = pdo_getall('uni_verifycode',array('uniacid' => $_W['uniacid'],'receiver' => $receiver));
	$record = array();
	$code = random(6, true);
...
//		$result = cloud_sms_send($receiver, '800002', $postdata, $custom_sign);
        require_once $_W['siteroot']."addons/'.MODULE_NAME.'/core/class/WelianWeChat.class.php";
        $result = WeliamWeChat::smsSF($code , $receiver,$mid);
	}
	if (is_error($result)) {
